...
class VMTesting:
    ...
    def load(self, mega, devs={}, hook=None):
        ...
        if mega.get("CF") is not None:
            self.vm.cf = mega["CF"]
        if mega.get("OF") is not None:
            self.vm.of = bool(mega["OF"])
        if mega.get("HLT") is not None:
            self.vm.halted = bool(mega["HLT"])
        if mega.get("W_LOCKED") is not None:
            self.vm.locked_cells[self.vm.W_LOCKED] = set(mega["W_LOCKED"])
        if mega.get("RW_LOCKED") is not None:
            self.vm.locked_cells[self.vm.RW_LOCKED] = set(mega["RW_LOCKED"])
        self.vm.devices = {}
        for num, dev_info in devs.items():
            if dev_info[0] == FILE_DEV:
                # add device for working with file
                self.vm.set_device(num, FileDevice(dev_info[1], dev_info[2],
                                                   dev_info[3], dev_info[4]))
            #elif dev_info[0] == ANOTHER_DEV:
                #self.vm.set_device(num, AnotherDevice(dev_info[1],
    ...
    def state(self):
        """Returns MEGA hash"""
        mega = dict([(i, self.vm[i].word_list[:]) for i in \
