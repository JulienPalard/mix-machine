...
# Grammar of mix's arguments (n - means that this function does self.next() in
# the end):
# + SYMBOL          ::==  string with length from 1 to 10 of digits or alphas
#                         (one alpha required)
...
# + ARGUMENT        ::== ( ADDR_PART IND_PART F_PART ) |
#                          if is_instruction(operation)
# +                      W_EXP |        # if operation in("EQU", "ORIG", "CON",
#                                                         "END")
...
class ArgumentParser:
    ...
    def try_any(seq):
        """
        Try all functions in seq and return not None result of one of
        them or None
        """
        for f in (seq):
            res = f()
    ...
    def try_number(self):
        if self.get() is not None and 1 <= len(self.get()) <= 10 \
                and self.get().isdigit():
            return int(self.get())
        else:
            return None

    def try_defined_symbol(self):
        # Program algorithm is so that difference of symbols destroied.
        # Init of symbol table - there parsed arguments of directives
        # which use only DEFINED_SYMBOLS with symbol table that created
        # from defined labels (see algorithm).
        return self.try_symbol()

    def try_future_symbol(self):
        # Assemble - there parsed arguments of instructions
        # which use any symbols with full symbol table.
        return self.try_symbol()
    ...
    def try_literal(self):
        ...
        if res is None:
            ...
            if self.get() != "=":
                ...
                if res == 0:
                    try:
                        # (self.line.argument[0] = "=")!!
                        if self.line.argument[1] == '-':
                            return_tuple = (0, -1)
                        else:
                            return_tuple = (0, +1)
                    except:
    ...
    def try_argument(self):
        if is_instruction(self.line.operation):
            ...
            # check if field fixed for this instruction and f_part is
            # different from default
            if is_field_fixed(self.line.operation) \
                    and f_part != get_codes(self.line.operation)[1]:
