...
class ListingLine:
    ...
    def word2str_addr_bytes(self):
        ...
        return "%s %04i %02i %02i %02i" \
            % tuple([sign] + [self.word[1] * 64 + self.word[2]] \
                        + self.word[3:])

    def __str__(self):
        print "__str__(%s, %s, %s)" \
            % (repr(self.addr), repr(self.word), repr(self.line))
        if self.line[0] == '*':
            return self.line
        if self.addr is not None:
            return "%4s | %16s | %s" \
                % (self.addr2str(), self.word2str(), self.line)
        else:
            return " " * 4 + " | " + " " * 16 + " | %s" \
                % (self.addr2str(), self.word2str(), self.line)

    def __cmp__(self, other):
        if self.addr != other.addr or self.word != other.word \
...
class Listing:
    ...
    def __init__(self, src_lines, asm_lines, memory, literals,
                 literals_address):
        "None added: so first line will have index 1"
        self.lines = map(lambda string: \
                             ListingLine(line=string.rstrip('\r\n')), \
                             src_lines)
    ...
    def create_listing(self):
        for asm_line in self.asm_lines:
            if asm_line.asm_address is not None:
                self.lines[asm_line.line_number - 1].addr = \
                    asm_line.asm_address
                self.lines[asm_line.line_number - 1].word = \
                    self.memory[asm_line.asm_address]
        for literal in self.literals:
            sign = "-" if literal[1] == -1 else ""
            self.lines.append(ListingLine(self.literals_address,
                                          self.memory[self.literals_address],
                                          "\tCON\t%s%i" % (sign, literal[0])))
